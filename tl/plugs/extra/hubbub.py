...
from tl.utils.url import posturl
from tl.lib.errors import NoSuchBotType, RequireError
...
from tl.imports import getfeedparser
from tl.api.hooks import get_hooks, post_hooks
...
try:
    from tl.utils.tinyurl import get_tinyurl
except ImportError:
    def get_tinyurl(url):
        return [url, ]
...
def hubbub_get(bot, event):
    logging.warn('incoming GET')
    #print dir(event.handler)
    #print dir(event.request)
    #print event.request
    #print event.request.arguments
    if event.request.arguments['hub.mode'][0] == 'unsubscribe':
        event.request.headers['Content-Type'] = 'text/plain'
        event.reply(event.request.arguments['hub.challenge'][0])
        event.finish()
        return  
    if event.request.arguments['hub.mode'][0] != 'subscribe': event.handler.send_error(400) ; return
    event.request.headers['Content-Type'] = 'text/plain'
    event.reply(event.request.arguments['hub.challenge'][0])
    event.finish()

## hubbub_post function

def hubbub_post(bot, event):
    """Handles new content notifications."""
    logging.warn("incoming POST")
    try: watcher.incoming(event.request.body)
    except IndexError: logging.error("hubbub plugin did not load properly")
    except Exception as ex:
        handle_exception()
        event.handler.send_error(500)
    event.finish()
...
class HubbubWatcher(PlugPersist):
    ...
    def watch(self, name):
        ...
        logging.info('started %s watch' % name)
...
def init():
    get_hooks.register("/api/hubbub", hubbub_get)
    post_hooks.register("/api/hubbub", hubbub_post)

## plugins shutdown

def shutdown():
    get_hooks.unregister("/api/hubbub")
    post_hooks.unregister("/api/hubbub")

## dummycb to make sure this plugin loads on startup
...
watcher = HubbubWatcher('hubbub', socket.gethostname())
...
def handle_hubbubsubscribe(bot, event):
    ...
    for name in event.args:
        ...
        response = subscribe(url, socket.gethostname())
        event.reply("subscription send: %s - %s" % (url, response.status))

cmnds.add('hb-subscribe', handle_hubbubsubscribe, ['USER',])
examples.add('hb-subscribe', 'subscribe to a feed', 'hb-subscribe ggzpreventie.nl https://github.com/feedbackflow/tl/commits/master.atom')
...
def handle_hubbubadd(bot, ievent):
    ...
    try: result = subscribe(url, socket.gethostname())
    except RequireError as ex: ievent.reply(str(ex)) ; return
...
examples.add('hb-add', 'hubbub-add <name> <url> to the watcher', 'hb-add ggzpreventie.nl https://github.com/feedbackflow/tl/commits/master.atom')
...
examples.add('hb-watch', 'hubbub-watch <name> [seconds to sleep] .. go watching <name>', 'hb-watch ggzpreventie')
...
examples.add('hb-start', 'hubbub-start <list of feeds> .. start a hubbub feed (per user/channel) ', 'hb-start ggzpreventie.nl')
...
examples.add('hb-stop', 'hubbub-stop <list of names> .. stop a hubbub feed (per user/channel) ', 'hb-stop ggzpreventie.nl')
...
examples.add('hb-channels', 'hb-channels <name> .. show channels', 'hb-channels ggzpreventie.nl')

## hb-addchannel command
...
examples.add('hb-addchannel', 'add a channel to a feeds watchlist', '1) hb-addchannel ggzpreventie.nl #dunkbots 2) hb-addchannel ggzpreventie.nl main #dunkbots')
...
examples.add('hb-setitems', 'set tokens of the itemslist (per user/channel)', 'hb-setitems ggzpreventie.nl author author link pubDate')
...
examples.add('hb-additem', 'add a token to the itemslist (per user/channel)', 'hb-additem ggzpreventie.nl link')
...
examples.add('hb-delitem', 'remove a token from the itemslist (per user/channel)', 'hb-delitem ggzpreventie.nl link')
...
examples.add('hb-markup', 'show markup list for a feed (per user/channel)', 'hb-markup ggzpreventie.nl')
...
examples.add('hb-addmarkup', 'add a markup option to the markuplist (per user/channel)', 'hb-addmarkup ggzpreventie.nl all-lines 1')
...
examples.add('hb-delmarkup', 'remove a markup option from the markuplist (per user/channel)', 'hb-delmarkup ggzpreventie.nl all-lines')
...
examples.add('hubbub-delchannel', 'remove a channel from a feeds watchlist', '1) hb-delchannel ggzoreventie.nl #dunkbots 2) hb-delchannel ggzpreventie.nl main #dunkbots')
...
examples.add('hb-stopwatch', 'hubbub-stopwatch <name> .. stop polling <name>', 'hb-stopwatch ggzpreventie.nl')
...
examples.add('hb-get', 'hubbub-get <name> .. get data from <name>', 'hb-get ggzpreventie.nl')
...
examples.add('hb-url', 'hb-url <name> .. get url from hubbub item', 'hb-url ggzpreventie.nl')
...
examples.add('hb-itemslist', 'hb-itemslist <name> .. get itemslist of <name> ', 'hb-itemslist ggzpreventie.nl')
...
examples.add('hb-scan', 'hb-scan <name> .. get possible items of <name> ', 'hb-scan ggzpreventie.nl')
...
examples.add('hb-register', 'hb-register .. register url and start it in one pass', 'hb-register tlrepo https://github.com/feedbackflow/tl/commits/master.atom')
