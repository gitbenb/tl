...
from tl.utils.generic import stripped
...
class Users(Persist):
    ...
    def allowed_event(self, event, perms, *args, **kwargs):
        return self.allowed(event.userhost, perms, *args, **kwargs)

    def allowed(self, userhost, perms, log=True, bot=None):
        """ check if user with userhosts is allowed to execute perm command """
        if not userhost: raise NoSuchUser(userhost)
