...
class NSAuth(Pdod):
    ...
    def sendstring(self, bot, txt):
        ...
        if not txt: logging.error("no txt provided") ; return
        nickservnick = self.get(bot.cfg.name, 'nickserv')
        if not nickservnick: logging.warn("no nickserv defined") ; return
