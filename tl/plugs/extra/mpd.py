...
#
# CHANGELOG
#  BHJTW 2012-09-15 - ported to TIMELINE
#
#  2011-02-20
...
def handle_mpd_playlist(bot, ievent):
    ...
    if result: ievent.reply("playlist: ", result, dot="\n")
    else: ievent.reply("no playlist")
...
def handle_mpd_del(bot, ievent):
    ...
    if result: ievent.reply("result is %s " % result)
    else: ievent.reply("no result returned")   
...
def handle_mpd_set_option(bot, ievent, option):
    ...
    if ievent.args:
        ...
        if val > 0: 
            mpd('%s %s' % (option, val))
            ievent.reply("%s option set to %s" % (option, val))
        else: ievent.reply('"off" is not 0 or positive integer' % ievent.args[0])
        return
    else:
        status = MPDDict(mpd('status'))
